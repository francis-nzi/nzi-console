...
| d-2 | Swap `PortalEntryRecords`'s form for `EmissionEntryForm` (audience `portal`) inside each section — authorised-factor handling, portal vehicle-lookup route, submit-to-review mapping | ⏳ |
