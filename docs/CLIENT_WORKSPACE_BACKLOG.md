...
| **Migration/deploy gate + resilient hot-path reads** — staging auto-applies migrations before cutover (fails closed); production is a hard `migrate:status` check that never auto-applies; adjunct reads in `getClientWorkspace` degrade to honest "unavailable" while essential reads still fail loudly | NZC-077 · `DESIGN_CONVENTIONS` §12 · brief `docs/_handoff_MIGRATION_DEPLOY_GATE_brief.md` | **Built — #178.** ↳ **Two steps remain open and are Francis's to apply:** the staging `preDeployCommand` diff in `DEPLOYMENT.md` (deployment surface, proposed not applied), and the production posture, which is wired to the live service at go-live | Go-live |
