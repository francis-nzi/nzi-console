...
| **Email consent capture** — an audited way to set `client_contacts.email_consent` (`unknown` → `granted` / `declined`), with basis, note and history. Production gate (a) for strategy reminder email: `unknown` holds, so no contact can be written to until this exists | NZC-076 · migration `0083` (column) · `0084` (events) · brief `docs/_handoff_STRATEGY_CONSENT_brief.md` | **Built — #176**, phase 1, staff console (migration `0084`, append-only consent events; gated by the existing `contact.manage`, so the matrix stays at v3). Phase 2 (portal self-serve) not built; `portal-self-serve` basis is reserved but not staff-recordable | Should |
...
| **Quantified emissions projection from strategies** — a consultant ESTIMATE per strategy (tCO₂e/yr or % of the benchmark in force), rolled up into a projected trajectory against target and measured actuals, with the plan-vs-target gap and an over-claim guard | NZC-078 · migration `0085` · brief `docs/_handoff_QUANTIFIED_PROJECTION_brief.md` | **Built — #182** (backend, roll-up, chart, over-claim guard; migration `0085`) **+ the estimate-entry form and one-click catalogue seed — #183**. Both acceptance points from #182 are now met: a consultant can enter an estimate in the strategy drawer, and seed it from the catalogue default with its provenance recorded. Never conflated with the assured footprint; report and portal inclusion are later phases | Stage 2 |
...
| **Portal surfacing of readiness** (read-only, M6.4) — overall + per-pillar maturity (`SrsPillarRadar` / `SrsMaturityBullets`), shortfall-ordered gaps via `gaps()`, each naming the client strategies that address it. **Live** (current completed assessment), the twin of the report's frozen statement | portal `srs-readiness` · brief `docs/_handoff_PORTAL_SRS_STATEMENT_brief.md` | **Built — #177** (no migration). Gated on `portal-readiness` retrospectively (NZC-080 — it shipped ungated). ⚠️ **Still owes its staging acceptance pass.** Acceptance template ready to run: `docs/STAGING_ACCEPTANCE_PORTAL_READINESS.md`, data via `npm run seed:portal-acceptance` (plan `docs/_handoff_PORTAL_ACCEPTANCE_PLAN.md`). A draft assessment is held back rather than shown as provisional scoring; no assessment shows an honest empty state, never 0% | Should |
...
| **Portal plan view** — the client-facing **live** Reduction Strategies plan on the portal (grouped by lever, `include_in_report` only, read-only). Distinct from the deadline *signals* in #164, which are counts + exceptions only (`PortalStrategiesReadModel`) | §G plan · `getPortalClientStrategies` | **Built — #173** (no migration; brief `docs/_handoff_PORTAL_PLAN_VIEW_brief.md`). Gated on `portal-plan` retrospectively (NZC-080 — it shipped ungated). ⚠️ **Still owes its staging acceptance pass.** Acceptance template ready to run: `docs/STAGING_ACCEPTANCE_PORTAL_PLAN.md`, data via `npm run seed:portal-acceptance` (plan `docs/_handoff_PORTAL_ACCEPTANCE_PLAN.md`). `owner` and `notes` are kept off the wire; the workspace toggle now reads "Show this to the client" | Should |
...
| **Migration/deploy gate + resilient hot-path reads** — staging auto-applies migrations before cutover (fails closed); production is a hard `migrate:status` check that never auto-applies; adjunct reads in `getClientWorkspace` degrade to honest "unavailable" while essential reads still fail loudly | NZC-077 · `DESIGN_CONVENTIONS` §12 · brief `docs/_handoff_MIGRATION_DEPLOY_GATE_brief.md` | **Built — #178; staging gate LIVE and proven** (set as the Render dashboard Pre-Deploy Command — the service is not Blueprint-managed, so the `render.yaml` line is future-proofing for a rebuild, not the active mechanism). It applied `0085` on #182 unattended. ↳ **Production remains open by design:** a hard `migrate:status` check, wired to the live service at go-live | Go-live |
...
| **Help system — Phase 0a: knowledge library** — NZI-wide (no `client_id`), duplicate-safe idempotent capture, two-tier `draft → internal → public`, three capabilities at matrix v4, Library + Review UI | NZC-081 · migrations `0086`/`0087` · design `docs/_handoff_HELP_SYSTEM_design.md` | **Built (#190)** — carries migrations `0086`/`0087`, applied to staging by the NZC-077 pre-deploy gate | Should |
| **Help system — Phase 0b: drawer + `?` affordance** — rendered by `TopBar`, so every page has it without wiring; page-aware context chip; §3.4 anatomy, four tabs, focus-managed, reduced-motion. Library/Review re-host the 0a components; Ask abstains honestly (no generation until Phase 1); Guide stubbed until 0c | NZC-081 · `docs/_handoff_HELP_PHASE0_brief.md` §0b | **Built (#191)** — no migration | Should |
| **Help system — Phase 0c: tour engine + seen-state** — declarative per-page tours (data, versioned), in-house coach-marks (spotlight, progress, Back/Next/Skip, don’t-show-again), focus-trapped and keyboard-driven; auto-run once per person per tour **version**, always replayable from the Guide tab; exemplar client-workspace tour | NZC-082 · migration `0088` · `docs/_handoff_HELP_PHASE0_brief.md` §0c | **Built (#192)** — carries migration `0088`, applied to staging by the NZC-077 pre-deploy gate | Should |
| **Help system — Phase 0d: grounding interface** — source-agnostic retrieval, citation enforced in the type, honest abstention as a first-class result, retrieved content tagged as data. Library wired as the concrete source; product docs declared as a **distinct corpus** with none indexed yet, and a source may only cite its own (NZC-084 — one approved entry must never appear as two corroborating citations) | NZC-083 · NZC-084 · `docs/_handoff_HELP_PHASE0_brief.md` §0d | **Built (#193)** — no migration. **No model call.** The NZC-084 corpus rule (`emits`, enforced in `retrieveGrounding`) was added after #193 was cut and lands with #194 | Should |
| **Help system — Phase 1: grounded answering** — the drawer answers from approved knowledge, cited. A model's references are resolved against the candidates it was given and one bad reference refuses the whole answer; the model is never called for a question retrieval could not source; answered / abstained / failed / waiting stay distinct, and the retrieved sources show even when no prose was written. Model injected behind an interface, so the rules hold with no key | NZC-085 · design `docs/_handoff_HELP_SYSTEM_design.md` §2, §5 | **Built (#194)** — no migration. Generation is **off until `ANTHROPIC_API_KEY` is set** in the staging dashboard — absent, the drawer retrieves and cites but writes nothing, and says so. The HTTP adapter is the one path no test exercises; first real key use is a supervised check | Should |
| **Staff portal preview ("Go to portal")** — read-only preview of a client's portal from the client Overview, under the **staff** identity (not impersonation). Renders the client's own components through the client's own read models, applies the portal feature gates, persistent "Staff preview · <Client> · read-only" banner, every open audited with the staff actor | NZC-087 · brief `docs/_handoff_PORTAL_PREVIEW_brief.md` | **Built — PR number pending** (no migration). Gated on the **existing** `support.portal_impersonate`; **no new capability, and the matrix stays at v4** — that capability already described this feature and had never been enforced | Should |
| ↳ **Portal enrolment + login revamp** — invite → single-use setup link → password/TOTP → `/portal/login` is erroring ("Register unavailable", "Sign-in could not be completed"). First check: `NZI_WRITE_API_ENABLED` is likely not `true` on staging — it is absent from `render.yaml`, and its absence returns 503 `WRITE_API_DISABLED` on exactly these writes | NZC-088 | **Parked** — not started. The staff preview does not depend on it | Should |
| ↳ Help **Phase 2** (permission-checked live-data read-model tools — the AI answers about the user's own data, as the asking user, from the same resolvers the UI uses) | design `docs/_handoff_HELP_SYSTEM_design.md` §2(c), §5 | **To build** — the grounding contract already admits it without change (`live-data` citations name the read model) | Should |
