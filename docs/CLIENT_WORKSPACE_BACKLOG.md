...
<<<<<<< HEAD
| **Help system — Phase 0d: grounding interface** — source-agnostic retrieval, citation enforced in the type, honest abstention as a first-class result, retrieved content tagged as data. Library wired as the concrete source; product docs declared as a **distinct corpus** with none indexed yet, and a source may only cite its own (NZC-084 — one approved entry must never appear as two corroborating citations). **No model call** | NZC-083 · NZC-084 · `docs/_handoff_HELP_PHASE0_brief.md` §0d | **Built — PR number pending** (no migration) | Should |
| ↳ Help **Phase 1** (grounded answering over library + docs) · **Phase 2** (permission-checked live-data read-model tools) | design `docs/_handoff_HELP_SYSTEM_design.md` §5 | **To build** — Phase 0 framework complete (0a–0d) | Should |
=======
| **Help system — Phase 0d: grounding interface** — source-agnostic retrieval, citation enforced in the type, honest abstention as a first-class result, retrieved content tagged as data. Library wired as the concrete source; product docs declared with no corpus indexed yet. **No model call** | NZC-083 · `docs/_handoff_HELP_PHASE0_brief.md` §0d | **Built — PR number pending** (no migration) | Should |
| **Help system — Phase 1: grounded answering** — the drawer answers from approved knowledge, cited. A model's references are resolved against the candidates it was given and one bad reference refuses the whole answer; the model is never called for a question retrieval could not source; answered / abstained / failed / waiting stay distinct, and the retrieved sources show even when no prose was written. Model injected behind an interface, so the rules hold with no key | NZC-085 · design `docs/_handoff_HELP_SYSTEM_design.md` §2, §5 | **Built — PR number pending** (no migration). Generation is **off until `ANTHROPIC_API_KEY` is set** in the staging dashboard — absent, the drawer retrieves and cites but writes nothing, and says so | Should |
...
>>>>>>> 6b7f6aa (feat(help): the drawer answers, and every citation is checked before you see it)
