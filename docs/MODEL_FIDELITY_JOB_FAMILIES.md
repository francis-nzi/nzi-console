...
3. **LCA reference module** — `apps/console/app/jobs/lca/`: own routing off `job.header.family === "lca"`,
   own stage machine. Behind a `job-module-lca` flag; `FamilyWorkspace` still serves lca when the flag is
   off. Prove it (acceptance gate, like each B/S slice).
   **Slice 1 (Model Register) built 5 Sep 2026, PR #94** — `docs/ACCEPTANCE_LCA_MODULE_SLICE1.md`.
   ...
   **L4 Gap-filling + calc engine + result snapshot + review** (built 5 Sep 2026 —
   ...
   `review_status='approved'`) → **L5 Scenarios** (built 5 Sep 2026 — `docs/ACCEPTANCE_LCA_MODULE_SLICE5.md`) → **L6 Charts** (built 5 Sep 2026 — `docs/ACCEPTANCE_LCA_MODULE_SLICE6.md`) → **L7 Report manifest + PCF labelling** (built 5 Sep 2026 — `docs/ACCEPTANCE_LCA_MODULE_SLICE7.md`; frozen factor citation in the snapshot, migration 0056; the family report reuses the R-track paged machinery + L6 charts; PCF jobs get the NZC-039 label). **The LCA reference module L1–L7 is complete.**
