...
export function PortalAccount() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { const response = await fetch("/api/portal/auth/password", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ currentPassword: form.get("currentPassword"), newPassword }) });
      ...
      setNotice({ message: response.ok ? "Password changed. Other portal sessions have been signed out; MFA remains active." : body.message ?? "Password change failed.", ok: response.ok });
      if (response.ok) { event.currentTarget.reset(); setPasswordLength(0); }
    } catch { setNotice({ message: "The secure password service could not be reached. Your password has not been changed.", ok: false }); }
