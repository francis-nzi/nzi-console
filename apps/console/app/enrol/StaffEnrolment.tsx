...
export function StaffEnrolment() {
  ...
  return <form onSubmit={submit} className="nz-invite-card" aria-busy={pending}>
    ...
    {token === "" ? <div className="nz-banner warn" role="alert">This enrolment link is incomplete. Ask your administrator for a new one.</div>
      : setup ? <><p>Scan this with your authenticator app for <b>{setup.email}</b>, then enter the six-digit code it shows.</p>{/* Drawn here, from the value already on the page: the URI carries the secret, so it never goes to a QR service, a
         remote image, or any attribute — only into the SVG's geometry. */}<div className="nz-enrol-qr"><div className="nz-enrol-qr-field" role="img" aria-label="Authenticator setup QR code"><QRCodeSVG value={setup.otpauthUri} size={184} level="M" marginSize={4} bgColor="#FFFFFF" fgColor="#0B1B2B" aria-hidden="true" focusable="false" /></div><small>Enrolling on this phone? <a href={setup.otpauthUri}>Open it in your authenticator</a> instead.</small></div><p className="nz-enrol-manual">Can’t scan? Enter this key manually</p><div className="nz-setup-key"><span>Setup key</span><strong className="num">{setup.totpSecret}</strong><button type="button" onClick={copySecret}>{copied ? "Copied" : "Copy key"}</button></div><label key="code" className="nz-fl">Authenticator code<input className="nz-inp num" name="code" inputMode="numeric" pattern="[0-9]{6}" maxLength={6} autoComplete="one-time-code" required disabled={pending} autoFocus /></label></>
