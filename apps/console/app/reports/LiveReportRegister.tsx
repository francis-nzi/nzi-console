...
export function LiveReportRegister() {
  ...
  return <section className="nz-report-register" aria-label="Immutable report register">
    ...
    {reports.length === 0 ? <div className="nz-register-empty"><i>0</i><div><b>No immutable report versions</b><span>Validate a reviewed snapshot to create the first controlled version.</span></div><Link className="nz-btn pri" href="/report-preview">Prepare report version</Link></div> : <>
      <div className="nz-panel nz-report-table"><table className="nz-tbl"><thead><tr><th>Report version</th><th>Job / client</th><th>Manifest</th><th>Status</th><th>Client review</th><th>Created</th><th><span className="nz-sr-only">Actions</span></th></tr></thead><tbody>{reports.map((report) => <tr key={report.reportVersionId}><td><b>{report.reportVersionId}</b><code>{report.dataHash.slice(0, 22)}…</code></td><td><Link className="nz-register-job" href={`/jobs/${report.jobId}`}>{report.jobNumber}</Link><div className="muted">{report.client}{report.reportingYear ? ` · ${report.reportingYear}` : ""}</div></td><td>CRP · v{report.manifestVersion}</td><td><span className={`nz-st ${report.status === "published" ? "done" : report.status === "validated" ? "est" : "need"}`}>{report.status}</span></td><td>{report.status === "published" ? <><b>{report.approvalCount}</b> approvals · {report.commentCount} messages</> : <span className="muted">Not released</span>}</td><td>{formatDate(report.createdAt)}</td><td><Link className="nz-btn" href={`/reports/${report.reportVersionId}`}>Open version</Link></td></tr>)}</tbody></table></div>
      <div className="nz-report-cards">{reports.map((report) => <article key={report.reportVersionId}><div><span className={`nz-st ${report.status === "published" ? "done" : report.status === "validated" ? "est" : "need"}`}>{report.status}</span><b>{report.reportVersionId}</b><code>{report.dataHash.slice(0, 22)}…</code></div><dl><div><dt>Job / client</dt><dd>{report.jobNumber} · {report.client}</dd></div><div><dt>Client review</dt><dd>{report.status === "published" ? `${report.approvalCount} approvals · ${report.commentCount} messages` : "Not released"}</dd></div><div><dt>Created</dt><dd>{formatDate(report.createdAt)}</dd></div></dl><Link className="nz-btn" href={`/reports/${report.reportVersionId}`}>Open report version</Link></article>)}</div>
    </>}
  </section>;
}
