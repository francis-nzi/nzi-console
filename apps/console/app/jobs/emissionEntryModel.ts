 ...
 */
// `entryUnitsForCategory`, `manualEntryHint` and `buildEmissionEntryFields` are retired (NZC-102).
// A category's units, its manual-entry wording and its field list are rows in the governed input
// spec, read server-side and rendered by `renderInputSpec`. They are not re-declared here: a second
// definition is the dual source the migration existed to remove, and it would drift silently
// because nothing would be reading it.
