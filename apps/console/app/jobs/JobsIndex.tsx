...
export function JobsIndex({ jobs, clients }: { jobs: FamilyJob[]; clients: Client[] }) {
  ...
  const firstClient = clients.find((client) => client.status !== "prospect")?.id ?? "";
    ...
    <div className="nz-body" style={{ paddingTop: 16 }}>
      ...
      {creating && <form className="nz-panel" style={{ padding: 18, marginBottom: 16 }} onSubmit={createJob}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(180px, 1fr))", gap: 14, marginTop: 16 }}>
          <label className="nz-fl" style={{ margin: 0 }}>Client<select className="nz-sel" required value={draft.clientId} onChange={(e) => setDraft({ ...draft, clientId: e.target.value })}>{clients.filter((client) => client.status !== "prospect").map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select></label>
          ...
          <label className="nz-fl" style={{ margin: 0 }}>Initial stage<input className="nz-inp" required value={draft.workflowStage} onChange={(e) => setDraft({ ...draft, workflowStage: e.target.value })} /></label>
