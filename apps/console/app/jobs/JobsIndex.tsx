...
export function JobsIndex({ jobs: allJobs, clients, clientId = null }: { jobs: FamilyJob[]; clients: Client[]; clientId?: string | null }) {
  ...
  return <AppShell rail={<WorkspaceRail sections={NAV} activeId="jobs" user={USER} />}>
    ...
    <div className="nz-body" style={{ paddingTop: 16 }}>
      <section className="nz-ops-hero"><div><span className="nz-eyebrow light">NZI delivery command</span><h2>{jobs.length===0?"No delivery jobs are recorded.":dueSoon?`${dueSoon} job${dueSoon===1?"":"s"} approaching a delivery milestone.`:"The evidenced portfolio has no immediate milestones."}</h2><p>{jobs.length===0?"Portfolio health, ownership and workflow assurance remain unavailable until the first governed job is created.":"Official job numbering, family-specific workflows and accountable ownership create one dependable operational view."}</p></div><div className="nz-ops-trust"><span><i>{jobs.length?"✓":"·"}</i> Official numbering</span><span><i>{jobs.length?"✓":"·"}</i> Named ownership</span><span><i>{jobs.length?"✓":"·"}</i> Audited workflow</span></div></section>
      ...
      {creating && <form className="nz-panel nz-job-create" onSubmit={createJob}>
        <div className="nz-job-create-head"><div><span className="nz-eyebrow">New job</span><b>Create job</b><div className="sub" style={{ marginTop: 4 }}>The official number is assigned only when creation commits, so abandoned drafts never create gaps.</div></div><span className="nz-st est">Number pending</span></div>
      ...
      <div className="nz-panel nz-engagement-table"><table className="nz-tbl"><thead><tr><th>Job</th><th>Family</th><th>Client</th><th>Title</th><th>Stage</th><th>Due</th><th>Owner</th><th className="num">Progress</th></tr></thead><tbody>{rows.map(({ header }) => <tr key={header.id} className="row"><td><a href={`/jobs/${header.id}`} className="nz-table-link">{header.number}</a></td><td><span className="nz-st est">{jobFamilyMeta[header.family].code}</span></td><td>{header.client}</td><td>{header.title}</td><td>{header.workflowStage}</td><td>{formatDate(header.dueDate)}</td><td>{header.owner}</td><td><span className="nz-job-progress"><i><span style={{width:`${header.progressPct}%`}}/></i><b className="num">{header.progressPct}%</b></span></td></tr>)}</tbody></table>{rows.length===0&&<div className="nz-engagement-empty"><b>{jobs.length!==0?"No jobs match this family":scopedClient?"No jobs for this client yet":"No jobs yet"}</b><span>{jobs.length!==0?"Choose another family filter to return to active delivery work.":scopedClient?"This client has no delivery jobs on record.":"Create the first governed job after an eligible client exists."}</span></div>}</div>
    </div>
  </AppShell>;
}
