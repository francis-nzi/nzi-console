...
import {VehicleBulkPanel} from "./VehicleBulkPanel";
...
function TargetPanel({jobId,reportingYear,target,notice}:{jobId:string;reportingYear:number;target:EmissionsTargetReadModel|null;notice:(n:{kind:"ok"|"warn";text:string})=>void}){
 const router=useRouter(),[value,setValue]=useState({baselineYear:target?.baselineYear??reportingYear,baselineTco2e:target?.baselineTco2e??0,interimYear:target?.interimYear??2030,interimReductionPercent:target?.interimReductionPercent??50,netZeroYear:target?.netZeroYear??2050}),[pending,setPending]=useState(false),[reason,setReason]=useState("");
 const rebaselining=target!==null&&(value.baselineYear!==target.baselineYear||value.baselineTco2e!==target.baselineTco2e);
 async function save(){setPending(true);const result=await putBrowserCommandWithReason<{version:number}>(`/api/isolated/jobs/${jobId}/emissions-target`,{...value,expectedVersion:target?.version??0},crypto.randomUUID(),rebaselining?reason:"");if(result.state==="success")setReason("");setPending(false);if(result.state==="success"){notice({kind:"ok",text:`Emissions target v${result.data.version} saved. It will be frozen into the next reviewed snapshot.`});router.refresh();}else notice({kind:"warn",text:errorText(result)});}
 ...
 return <section className="nz-panel nz-config-panel"><div className="nz-config-head"><div><span className="nz-eyebrow">Assured reporting input</span><b>Reduction pathway target</b><div className="sub">Baseline, interim reduction and net-zero milestone used by the shared report chart.</div></div><span className={`nz-st ${target?"done":"est"}`}>{target?`Version ${target.version}`:"Not configured"}</span></div><div className="nz-config-grid target">{field("Baseline year","baselineYear")}{field("Baseline tCO₂e","baselineTco2e","any")}{field("Interim year","interimYear")}{field("Interim reduction %","interimReductionPercent","any")}{field("Net-zero year","netZeroYear")}</div>{rebaselining?<div className="nz-banner warn" role="status" style={{margin:"0 16px 12px"}}><div><b>Changing a saved baseline is a re-baseline.</b><div style={{marginTop:4}}>It needs a reason, recorded on the baseline record and in the audit log (baseline.rebaseline; a Consultant on their own clients only).</div><label className="nz-fl" style={{marginTop:8}}><span>Reason for the re-baseline</span><input className="nz-inp" value={reason} onChange={e=>setReason(e.target.value)} aria-label="Reason for the target re-baseline"/></label></div></div>:null}<div className="nz-config-actions"><button className="nz-btn pri" disabled={pending||(rebaselining&&!reason.trim())} onClick={save}>{pending?"Saving…":"Save target"}</button></div></section>;
}
