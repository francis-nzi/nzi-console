...
import type { DatasetOption,EmissionsTargetReadModel,FactorOption,SiteOption,ScopeQaReadiness,ScopeRowReadModel } from "@nzi/contracts";
...
export default async function JobPage({ params }: { params: Promise<{ jobId: string }> }) {
  ...
  const [result, scopeRows, factors,target,sites] = await Promise.all([
  ...
  return <ScreenState result={result}>{(data) => {
    ...
    if (!job) notFound();
    return job.header.family === "crp" ? <ScreenState result={scopeRows}>{(scopeData) => <ScreenState result={factors}>{(factorData) => <ScreenState result={target}>{targetData=><ScreenState result={sites}>{siteData=><CrpScopeWorkspace job={job} rows={scopeData.rows} qa={scopeData.qa} factors={factorData.factors} datasets={factorData.datasets} target={targetData.target} sites={siteData.sites}/>}</ScreenState>}</ScreenState>}</ScreenState>}</ScreenState> : <FamilyWorkspace job={job} />;
  }}</ScreenState>;
}
