import {createEmissionSource,listJobEmissionSourceRegister,listJobReportingMonths,withTenantRead} from "@nzi/isolated-backend";
...
export async function GET(_request:Request,{params}:{params:Promise<{jobId:string}>}){try{const {jobId}=await params,{pool,organisationId}=requireIsolatedApiContext();return Response.json(await withTenantRead(pool,organisationId,async db=>({...await listJobEmissionSourceRegister(db,jobId),reportingMonths:await listJobReportingMonths(db,jobId)})));}catch(error){return apiFailure(error);}}
