...
export async function loadScreen<T>(key: ScreenKey, fixtureValue: unknown): Promise<ScreenResult<T>> {
  ...
  return requestScreen<T>(key, () => fetch(`${base}/api/isolated/${key}`, { cache: "no-store", headers: { "x-request-id": requestId, ...(cookie ? { cookie } : {}) } }), { requestId });
}
