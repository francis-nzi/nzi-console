...
export default async function ClientPage({ params }: { params: Promise<{ clientId: string }> }) {
  ...
  return <ScreenState result={clientResult}>{(clientData) => {
    const client = clientData.clients.find((item) => item.id === clientId);
    if (!client) notFound();
    return <ScreenState result={jobResult}>{(jobData) => <ClientWorkspace client={client} jobs={jobData.jobs.filter((job) => job.header.clientId === client.id)} />}</ScreenState>;
  }}</ScreenState>;
}

function ClientWorkspace({ client, jobs }: { client: ClientScreenReadModel; jobs: JobScreenReadModel[] }) {
  ...
  return <AppShell rail={<WorkspaceRail sections={NAV} activeId="clients" user={USER} />}>
    ...
    <div className="nz-head"><div className="nz-client-head"><div className="nz-client-monogram">{client.name.split(/\s+/).slice(0, 2).map((word) => word[0]).join("")}</div><div className="nz-client-identity"><div><h1>{client.name}</h1><span className={`nz-st ${meta.cls}`}>{meta.label}</span></div><p className="sub">{client.sector} · {client.location} · Account owner {client.owner}</p></div><div className="nz-head-actions"><Link className="nz-btn" href="/platform">Portal access</Link><Link className="nz-btn pri" href={`/jobs?client=${client.id}`}>Create job</Link></div></div></div>
    <div className="nz-body" style={{ paddingTop: 16 }}>
      <div className="nz-metrics"><Metric label="Open jobs" value={String(open.length)} note={`${jobs.length} total engagements`} /><Metric label="Operational sites" value={String(client.sites.length)} note="Tenant-owned locations" /><Metric label="Rows awaiting review" value={String(reviewGaps)} note="Across CRP engagements" /><Metric label="Next report" value={client.nextReportDue || "Not scheduled"} note="Client reporting calendar" /></div>
      <div className="nz-client-grid"><div className="nz-client-main">
        ...
        <section className="nz-panel"><Head title="Account overview" /><div style={{ padding: "6px 16px 12px" }}><Row label="Member since" value={client.memberSince} /><Row label="Account owner" value={client.owner} /><Row label="Year-on-year" value={client.yoy ?? "Not available"} /><Row label="Data completeness" value={`${client.completeness}%`} /></div></section>
...
function Head({ title, right }: { title: string; right?: React.ReactNode }) { return <div style={{ display: "flex", padding: "13px 16px", borderBottom: "1px solid var(--line2)", alignItems: "center" }}><h2 style={{ fontSize: 15, margin: 0 }}>{title}</h2>{right ? <div className="sub" style={{ marginLeft: "auto", fontSize: 12 }}>{right}</div> : null}</div>; }
