...
describe("staff workspace acceptance contracts",()=>{
  ...
  it("never hard-codes a Xero connection — the status is derived (NZC-069)",()=>{const source=read("packages/isolated-backend/src/readModels.ts");assert.doesNotMatch(source,/xeroStatus:\s*\{\s*state:\s*"connected"\s*,/);assert.match(source,/if \(!integration\.configured\) return \{ state: "not_configured", label: "Not connected" \}/);assert.match(source,/const synced = documents\.some\(\(document\) => document\.xero\.status === "synced" && document\.xero\.lastSyncedAt !== null\)/,"Connected needs evidence of a real sync");assert.match(source,/if \(!synced\) return \{ state: "awaiting_sync"/)});
