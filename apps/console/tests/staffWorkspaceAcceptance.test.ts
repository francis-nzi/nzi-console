...
const clientWorkspace=["apps/console/app/clients/[clientId]/ClientWorkspaceView.tsx","apps/console/app/clients/[clientId]/ClientSites.tsx","apps/console/app/clients/[clientId]/FigureEvidence.tsx"];

describe("staff workspace acceptance contracts",()=>{
  it("renders each client from its own resolved figures and sites — no shared mock evidence or sites (NZC-005/070)",()=>{for(const file of ["apps/console/app/clients/[clientId]/page.tsx",...clientWorkspace]){const source=read(file);assert.doesNotMatch(source,/clientFigureEvidence|clientSites|documentHistory|financials/,file)}});
