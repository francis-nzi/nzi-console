...
// precondition (fail loud, never a silent skip). The one conditional skip is
// for the flag not yet being live on the target — removed at the flip PR.
...
test.describe("R5a — report audit appendices", () => {
  ...
  test("Appendix 1 (Full Emissions Audit) and Appendix 2 (by Site, Scope & Category) render from the frozen snapshot", async ({ page }) => {
    ...
    test.skip((await appendix.count()) === 0, "report-paged not enabled on target (no .report-appendix)");
  ...
  test("the audit table is a real paged-media table — repeating thead, row-atomic breaks", async ({ page }) => {
    ...
    const table = page.locator("table.report-audit-table").first();
    test.skip((await table.count()) === 0, "report-paged not enabled on target");
  ...
  test("passes the axe baseline and holds the column with the appendix present", async ({ page }) => {
    await openReportVersion(page);
    test.skip((await page.locator(".report-appendix").count()) === 0, "report-paged not enabled on target");
