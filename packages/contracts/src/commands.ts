...
export const commandDefinitions: { [K in CommandKey]: CommandDefinition<K> } = {
  ...
  "job.create": { key: "job.create", label: "Create job", permission: "job.manage", reasonRequired: false, transaction: "number allocation + job + audit + outbox + idempotency", auditAction: "job_created", validate: (input, context) => { const issues = baseIssues(context, false); required(issues, "clientId", input.clientId); required(issues, "title", input.title); required(issues, "workflowStage", input.workflowStage); required(issues, "owner", input.owner); if (!oneOf(input.family, ["crp", "consultancy", "lca", "pcf", "training"] as const)) issues.push({ field: "family", code: "INVALID", message: "Job family is invalid." }); if (!isoDate(input.startDate)) issues.push({ field: "startDate", code: "INVALID", message: "Enter the start date as dd/mm/yyyy." }); if (!isoDate(input.dueDate)) issues.push({ field: "dueDate", code: "INVALID", message: "Enter the due date as dd/mm/yyyy." }); if (isoDate(input.startDate) && isoDate(input.dueDate) && input.dueDate < input.startDate) issues.push({ field: "dueDate", code: "INVALID_RANGE", message: "Due date must not precede start date." }); return issues; } },
