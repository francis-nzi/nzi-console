...
export type CommandInputMap = {
  ...
  "assurance.gap.resolve": { jobId: string; gapKey: string; flagType: "yoy_movement" | "completeness" | "zero_blank" | "unmapped"; scopeRowId?: string | null; reason: string };
...
export const commandDefinitions: { [K in CommandKey]: CommandDefinition<K> } = {
  ...
  "assurance.gap.resolve": { key:"assurance.gap.resolve",label:"Resolve a data-integrity gap",permission:"emissions.review",reasonRequired:false,transaction:"gap resolution upsert + audit + outbox",auditAction:"assurance_gap_resolved",validate:(input,context)=>{const issues=baseIssues(context,false);required(issues,"jobId",input.jobId);required(issues,"gapKey",input.gapKey);required(issues,"reason",input.reason);if(text(input.reason)&&input.reason.trim().length>1000)issues.push({field:"reason",code:"TOO_LONG",message:"A resolution reason must be 1,000 characters or fewer."});if(!oneOf(input.flagType,["yoy_movement","completeness","zero_blank","unmapped"] as const))issues.push({field:"flagType",code:"INVALID",message:"Unknown gap flag type."});if(input.scopeRowId!=null&&typeof input.scopeRowId!=="string")issues.push({field:"scopeRowId",code:"INVALID",message:"Scope row id is invalid."});return issues;} },
