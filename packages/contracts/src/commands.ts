...
export type CommandKey =
  ...
  | "commercial.quote.send"
  | "commercial.quote.approve"
  | "commercial.quote.accept"
  | "commercial.quote.revise"
  | "commercial.quote.email"
  | "commercial.invoice.raise"
  | "commercial.invoice.send"
  | "commercial.creditNote.create"
  | "commercial.creditNote.apply"
  | "commercial.document.email"
  | "commercial.invoice.markPaid"
...
export type CommandInputMap = {
  ...
  "assurance.gap.resolve": { jobId: string; gapKey: string; flagType: "yoy_movement" | "completeness" | "zero_blank" | "unmapped"; scopeRowId?: string | null; reason: string; expectedVersion: number };
  ...
  "site.create": {jobId:string;name:string;inServiceFrom?:string;isRegisteredOffice?:boolean};
  "site.edit": {siteId:string;name:string;inServiceFrom:string;expectedVersion:number};
  ...
  "commercial.quote.create": { clientId: string; title: string; total: number; currency: string; validUntil: string };
  "commercial.quote.send": { quoteId: string; expectedVersion: number };
  "commercial.quote.approve": { quoteId: string; expectedVersion: number };
  "commercial.quote.accept": { quoteId: string; expectedVersion: number };
  "commercial.quote.revise": { quoteId: string; expectedVersion: number; total: number; validUntil: string };
  "commercial.quote.email": { quoteId: string; expectedVersion: number; recipient: string };
  "commercial.invoice.raise": { clientId: string; quoteId?: string; title: string; currency: string; lineItems: Array<{ description: string; quantity: number; unitPrice: number }> };
  "commercial.invoice.send": { invoiceId: string; expectedVersion: number };
  "commercial.creditNote.create": { clientId: string; invoiceId?: string; jobId?: string; reason: string; amount: number; currency: string };
  "commercial.creditNote.apply": { creditNoteId: string; expectedVersion: number };
  "commercial.document.email": { documentId: string; documentType: "quote" | "invoice" | "credit_note"; recipient: string };
  "commercial.invoice.markPaid": { invoiceId: string; expectedVersion: number; xeroPaymentRef: string };
...
export const commandDefinitions: { [K in CommandKey]: CommandDefinition<K> } = {
  ...
  "site.create":{key:"site.create",label:"Create client site",permission:"emissions.data.edit",reasonRequired:false,transaction:"client site + audit + outbox",auditAction:"client_site_created",validate:(input,context)=>{const issues=baseIssues(context,false);required(issues,"jobId",input.jobId);required(issues,"name",input.name);if(input.inServiceFrom!=null&&!isoDate(input.inServiceFrom))issues.push({field:"inServiceFrom",code:"INVALID",message:"In-service date must use YYYY-MM-DD."});return issues;}},
  "site.edit":{key:"site.edit",label:"Edit client site",permission:"emissions.data.edit",reasonRequired:false,transaction:"versioned client site + audit + outbox + idempotency",auditAction:"client_site_updated",validate:(input,context)=>{const issues=baseIssues(context,false);required(issues,"siteId",input.siteId);required(issues,"name",input.name);if(!isoDate(input.inServiceFrom))issues.push({field:"inServiceFrom",code:"INVALID",message:"In-service date must use YYYY-MM-DD."});if(!positive(input.expectedVersion))issues.push({field:"expectedVersion",code:"INVALID",message:"Expected version must be positive."});return issues;}},
  "site.registeredOffice":{key:"site.registeredOffice",label:"Set registered office",permission:"emissions.data.edit",reasonRequired:false,transaction:"registered-office constraint + audit + outbox + idempotency",auditAction:"client_site_registered_office_changed",validate:(input,context)=>{const issues=baseIssues(context,false);required(issues,"siteId",input.siteId);if(typeof input.isRegisteredOffice!=="boolean")issues.push({field:"isRegisteredOffice",code:"INVALID",message:"Registered-office flag must be true or false."});if(!positive(input.expectedVersion))issues.push({field:"expectedVersion",code:"INVALID",message:"Expected version must be positive."});return issues;}},
  "site.vacate":{key:"site.vacate",label:"Vacate client site",permission:"emissions.data.edit",reasonRequired:false,transaction:"effective-dated site lifecycle + audit + outbox + idempotency",auditAction:"client_site_vacated",validate:(input,context)=>{const issues=baseIssues(context,false);required(issues,"siteId",input.siteId);if(!isoDate(input.effectiveDate))issues.push({field:"effectiveDate",code:"INVALID",message:"Vacated effective date must use YYYY-MM-DD."});if(!positive(input.expectedVersion))issues.push({field:"expectedVersion",code:"INVALID",message:"Expected version must be positive."});return issues;}},
  "site.reinstate":{key:"site.reinstate",label:"Reinstate client site",permission:"emissions.data.edit",reasonRequired:false,transaction:"effective-dated site lifecycle + audit + outbox + idempotency",auditAction:"client_site_reinstated",validate:(input,context)=>{const issues=baseIssues(context,false);required(issues,"siteId",input.siteId);if(!positive(input.expectedVersion))issues.push({field:"expectedVersion",code:"INVALID",message:"Expected version must be positive."});return issues;}},
  ...
  "commercial.quote.create": { key: "commercial.quote.create", label: "Create quote", permission: "financials.edit", reasonRequired: false, transaction: "quote + version + audit + outbox + idempotency", auditAction: "quote_created", validate: (input, context) => { const issues = baseIssues(context, false); required(issues, "clientId", input.clientId); required(issues, "title", input.title); if (input.total < 0) issues.push({ field: "total", code: "INVALID", message: "Total must be zero or greater." }); return issues; } },
  "commercial.quote.send": { key: "commercial.quote.send", label: "Send quote", permission: "financials.edit", reasonRequired: false, transaction: "quote status + audit + outbox + idempotency", auditAction: "quote_sent", validate: (input, context) => { const issues = baseIssues(context, false); required(issues, "quoteId", input.quoteId); if (!positive(input.expectedVersion)) issues.push({ field: "expectedVersion", code: "INVALID", message: "Expected version must be positive." }); return issues; } },
  "commercial.quote.approve": { key: "commercial.quote.approve", label: "Approve quote", permission: "financials.edit", reasonRequired: false, transaction: "quote status + audit + outbox + idempotency", auditAction: "quote_approved", validate: (input, context) => [...baseIssues(context, false), ...(text(input.quoteId) ? [] : [{ field: "quoteId", code: "REQUIRED", message: "quoteId is required." }]), ...(positive(input.expectedVersion) ? [] : [{ field: "expectedVersion", code: "INVALID", message: "Expected version must be positive." }])] },
  "commercial.quote.accept": { key: "commercial.quote.accept", label: "Accept quote", permission: "financials.edit", reasonRequired: false, transaction: "quote + job conversion + audit + outbox + idempotency", auditAction: "quote_accepted", validate: (input, context) => [...baseIssues(context, false), ...(text(input.quoteId) ? [] : [{ field: "quoteId", code: "REQUIRED", message: "quoteId is required." }]), ...(positive(input.expectedVersion) ? [] : [{ field: "expectedVersion", code: "INVALID", message: "Expected version must be positive." }])] },
  "commercial.quote.revise": { key: "commercial.quote.revise", label: "Revise quote", permission: "financials.edit", reasonRequired: false, transaction: "quote version + audit + outbox + idempotency", auditAction: "quote_revised", validate: (input, context) => [...baseIssues(context, false), ...(text(input.quoteId) ? [] : [{ field: "quoteId", code: "REQUIRED", message: "quoteId is required." }]), ...(positive(input.expectedVersion) ? [] : [{ field: "expectedVersion", code: "INVALID", message: "Expected version must be positive." }])] },
  "commercial.quote.email": { key: "commercial.quote.email", label: "Email quote", permission: "financials.edit", reasonRequired: false, transaction: "quote email event + audit + outbox + idempotency", auditAction: "quote_emailed", validate: (input, context) => [...baseIssues(context, false), ...(text(input.quoteId) ? [] : [{ field: "quoteId", code: "REQUIRED", message: "quoteId is required." }]), ...(text(input.recipient) ? [] : [{ field: "recipient", code: "REQUIRED", message: "recipient is required." }])] },
  "commercial.invoice.raise": { key: "commercial.invoice.raise", label: "Raise invoice", permission: "financials.edit", reasonRequired: false, transaction: "invoice + line items + number + audit + outbox + idempotency", auditAction: "invoice_raised", validate: (input, context) => [...baseIssues(context, false), ...(text(input.clientId) ? [] : [{ field: "clientId", code: "REQUIRED", message: "clientId is required." }]), ...(input.lineItems.length ? [] : [{ field: "lineItems", code: "REQUIRED", message: "At least one line item is required." }])] },
  "commercial.invoice.send": { key: "commercial.invoice.send", label: "Send invoice", permission: "financials.edit", reasonRequired: false, transaction: "invoice status + audit + outbox + idempotency", auditAction: "invoice_sent", validate: (input, context) => [...baseIssues(context, false), ...(text(input.invoiceId) ? [] : [{ field: "invoiceId", code: "REQUIRED", message: "invoiceId is required." }]), ...(positive(input.expectedVersion) ? [] : [{ field: "expectedVersion", code: "INVALID", message: "Expected version must be positive." }])] },
  "commercial.creditNote.create": { key: "commercial.creditNote.create", label: "Create credit note", permission: "financials.edit", reasonRequired: false, transaction: "credit note + audit + outbox + idempotency", auditAction: "credit_note_created", validate: (input, context) => [...baseIssues(context, false), ...(text(input.clientId) ? [] : [{ field: "clientId", code: "REQUIRED", message: "clientId is required." }]), ...(text(input.reason) ? [] : [{ field: "reason", code: "REQUIRED", message: "reason is required." }])] },
  "commercial.creditNote.apply": { key: "commercial.creditNote.apply", label: "Apply credit note", permission: "financials.edit", reasonRequired: false, transaction: "credit note + invoice balance + audit + outbox + idempotency", auditAction: "credit_note_applied", validate: (input, context) => [...baseIssues(context, false), ...(text(input.creditNoteId) ? [] : [{ field: "creditNoteId", code: "REQUIRED", message: "creditNoteId is required." }]), ...(positive(input.expectedVersion) ? [] : [{ field: "expectedVersion", code: "INVALID", message: "Expected version must be positive." }])] },
  "commercial.document.email": { key: "commercial.document.email", label: "Email commercial document", permission: "financials.edit", reasonRequired: false, transaction: "document email event + audit + outbox + idempotency", auditAction: "commercial_document_emailed", validate: (input, context) => [...baseIssues(context, false), ...(text(input.documentId) ? [] : [{ field: "documentId", code: "REQUIRED", message: "documentId is required." }]), ...(text(input.recipient) ? [] : [{ field: "recipient", code: "REQUIRED", message: "recipient is required." }])] },
  "commercial.invoice.markPaid": { key: "commercial.invoice.markPaid", label: "Reconcile invoice payment", permission: "financials.edit", reasonRequired: false, transaction: "invoice reconciliation + audit + outbox + idempotency", auditAction: "invoice_marked_paid", validate: (input, context) => [...baseIssues(context, false), ...(text(input.invoiceId) ? [] : [{ field: "invoiceId", code: "REQUIRED", message: "invoiceId is required." }]), ...(text(input.xeroPaymentRef) ? [] : [{ field: "xeroPaymentRef", code: "REQUIRED", message: "xeroPaymentRef is required." }])] },
  ...
  "lca.lineItem.create": { key: "lca.lineItem.create", label: "Create LCA line item", permission: "emissions.data.edit", reasonRequired: false, transaction: "line item + audit + outbox + idempotency", auditAction: "lca_line_item_created", validate: (input, context) => { const issues = [...baseIssues(context, false), ...lcaLineItemIssues(input)]; required(issues, "jobId", input.jobId); required(issues, "assessmentId", input.assessmentId); return issues; } },
