...
export type CommandInputMap = {
  ...
  "site.create": {jobId?:string|null;clientId?:string|null;name:string;inServiceFrom?:string|null;isRegisteredOffice?:boolean;floorAreaM2?:number|null};
  "site.edit": {siteId:string;name:string;inServiceFrom:string|null;expectedVersion:number};
...
export const commandDefinitions: { [K in CommandKey]: CommandDefinition<K> } = {
  ...
  "site.create":{key:"site.create",label:"Create client site",permission:"emissions.data.edit",reasonRequired:false,transaction:"client site + audit + outbox + idempotency",auditAction:"client_site_created",validate:(input,context)=>{const issues=baseIssues(context,false);required(issues,"name",input.name);if(text(input.jobId)===text(input.clientId))issues.push({field:"clientId",code:"INVALID",message:"Create a site from exactly one of a job or a client."});if(text(input.clientId)&&input.inServiceFrom===undefined)issues.push({field:"inServiceFrom",code:"REQUIRED",message:"State the in-service date, or mark the site as in service from before records."});if(input.inServiceFrom!=null&&!isoDate(input.inServiceFrom))issues.push({field:"inServiceFrom",code:"INVALID",message:"In-service date must be a real date (YYYY-MM-DD)."});if(input.floorAreaM2!=null&&!positiveArea(input.floorAreaM2))issues.push({field:"floorAreaM2",code:"INVALID",message:"Floor area must be greater than zero."});return issues;}},
  "site.edit":{key:"site.edit",label:"Edit client site",permission:"emissions.data.edit",reasonRequired:false,transaction:"versioned client site + audit + outbox + idempotency",auditAction:"client_site_updated",validate:(input,context)=>{const issues=baseIssues(context,false);required(issues,"siteId",input.siteId);required(issues,"name",input.name);if(input.inServiceFrom!==null&&!isoDate(input.inServiceFrom))issues.push({field:"inServiceFrom",code:"INVALID",message:"In-service date must be a real date (YYYY-MM-DD)."});if(!positive(input.expectedVersion))issues.push({field:"expectedVersion",code:"INVALID",message:"Expected version must be positive."});return issues;}},
  ...
  "site.floorArea.record":{key:"site.floorArea.record",label:"Record site floor area",permission:"emissions.data.edit",reasonRequired:false,transaction:"effective-dated floor-area record + versioned site + audit + outbox + idempotency",auditAction:"client_site_floor_area_recorded",validate:(input,context)=>{const issues=baseIssues(context,false);required(issues,"siteId",input.siteId);if(!positiveArea(input.floorAreaM2))issues.push({field:"floorAreaM2",code:"INVALID",message:"Floor area must be greater than zero."});if(input.effectiveFrom!==null&&!isoDate(input.effectiveFrom))issues.push({field:"effectiveFrom",code:"INVALID",message:"Effective-from date must be a real date (YYYY-MM-DD)."});if(!positive(input.expectedVersion))issues.push({field:"expectedVersion",code:"INVALID",message:"Expected version must be positive."});return issues;}},
