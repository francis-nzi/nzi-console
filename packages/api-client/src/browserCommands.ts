...
/** A governed change (e.g. a re-baseline) carries its reason in `x-command-reason`. */
export function patchBrowserCommandWithReason<T>(path: string, input: unknown, idempotencyKey: string, reason: string, transport: BrowserCommandTransport = fetch) { return sendBrowserCommand<T>("PATCH", path, input, idempotencyKey, transport, reason); }
...
export function putBrowserCommandWithReason<T>(path: string, input: unknown, idempotencyKey: string, reason: string, transport: BrowserCommandTransport = fetch) { return sendBrowserCommand<T>("PUT", path, input, idempotencyKey, transport, reason); }
