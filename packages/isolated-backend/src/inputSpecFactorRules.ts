...
function toRule(row: Row): FactorRule | null {
  ...
  switch (row.rule_kind) {
    ...
    case "sub-flow":
      // A sub-flow carries no base of its own: the base is whatever the referenced flow resolves.
      return row.sub_flow_category && row.suffix_code
        ? {
          kind: "sub-flow", ruleKey: row.rule_key, ordering: row.ordering,
          subFlowCategory: row.sub_flow_category, suffixCode: row.suffix_code,
        }
        : null;
    case "enriched":
...
const SELECT = `SELECT category_code, rule_key, ordering, rule_kind, factor_base,
                       ...
                       sub_flow_category, enrichment_source, enrichment_key_field
