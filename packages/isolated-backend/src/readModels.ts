...
import { listLevers, listLibraryStrategies, listClientStrategies } from "./reductionStrategies";
...
import { listRollforwardOrigins, loadRollforwardTarget, selectPriorJobByPeriod } from "./rollforwardSelection";
...
export type StaffRoleReadModel={id:StaffRole;name:string;members:number;matrixVersion:number;permissions:string[];restricted:string[]};
...
export type ClientWorkspaceReadModel = {
  ...
  /**
   * The reduction plan: the levers, the Admin-managed library, and what this client holds.
   * Both travel together because the library drawer has to mark what is already assigned,
   * and a plan whose levers were fetched separately could disagree with itself mid-render.
   */
...
/**
 * The audit trail a staff user may see (PERMISSION_MATRIX.md audit.view): none without
 * the capability; own clients only for a Consultant or Finance user; all for Admin/Reviewer.
 */
export async function auditEventsFor(db:Queryable,holder:{userId:string;capabilities:readonly CapabilityGrant[]},limit=100):Promise<AuditEventReadModel[]>{
  const scope=holder.capabilities.find(grant=>grant.capability==="audit.view")?.scope;
  if(!scope)return[];
  return listAuditEvents(db,limit,{ownerUserId:scope==="own_clients"?holder.userId:null});
}
...
export async function listScopeRowRollforwardPreview(db:Queryable,jobId:string):Promise<ScopeRowRollforwardPreview>{
  // Two separate questions (NZC-101): the recorded lineage, and the candidate for the next
  // rollforward. See rollforwardSelection.ts.
  const t=await loadRollforwardTarget(db,jobId);
  if(!t)return{priorJob:null,origins:[],rows:[]};
  const origins=await listRollforwardOrigins(db,jobId,"scopeRow");
  const p=await selectPriorJobByPeriod(db,t,jobId,"scopeRow");
  if(!p)return{priorJob:null,origins,rows:[]};
...
export async function listReviewedCrpSnapshots(db:Queryable,jobId:string):Promise<ReviewedCrpSnapshotReadModel[]>{const {rows}=await db.query<SnapshotRow&{approved_by?:string|null;approved_at?:Date|string|null}>(`SELECT snapshot_id,job_id,snapshot_version,job_version,data_hash,payload_json,created_by,created_at,approved_by,approved_at FROM nzi_console.reviewed_crp_snapshots WHERE job_id=$1 ORDER BY snapshot_version DESC`,[jobId]);return rows.map(row=>({id:row.snapshot_id,jobId:row.job_id,jobNumber:row.payload_json.jobNumber,client:row.payload_json.client,reportingYear:row.payload_json.reportingYear,version:row.snapshot_version,jobVersion:row.job_version,createdAt:row.created_at instanceof Date?row.created_at.toISOString():String(row.created_at),createdBy:row.created_by,approvedBy:row.approved_by??null,approvedAt:row.approved_at==null?null:row.approved_at instanceof Date?row.approved_at.toISOString():String(row.approved_at),dataHash:row.data_hash,target:row.payload_json.target??null,intensityTarget:row.payload_json.intensityTarget??null,annualComparison:row.payload_json.annualComparison??[],sections:row.payload_json.sections??resolveReportSections([]),gapResolutions:row.payload_json.gapResolutions??[],provenance:row.payload_json.provenance??null,measurements:row.payload_json.measurements}));}

type ReportReleaseColumns={signee_name?:string|null;signee_job_title?:string|null;client_logo_asset_id?:string|null};
const releaseFields=(row:ReportReleaseColumns)=>({signee:row.signee_name?{name:row.signee_name,jobTitle:row.signee_job_title??null}:null,clientLogoAssetId:row.client_logo_asset_id??null});
type PublishedReportRow=SnapshotRow&ReportReleaseColumns&{report_version_id:string;manifest_version:number;report_data_hash:string;published_at:Date|string};
type ReportVersionDetailRow=SnapshotRow&ReportReleaseColumns&{report_version_id:string;status:CrpReportVersionReadModel["status"];manifest_version:number;report_data_hash:string;published_at:Date|string|null};
export async function getCrpReportVersion(db:Queryable,reportVersionId:string):Promise<CrpReportVersionReadModel|null>{const {rows}=await db.query<ReportVersionDetailRow>(`SELECT r.report_version_id,r.status,r.manifest_version,r.data_hash AS report_data_hash,r.published_at,r.signee_name,r.signee_job_title,r.client_logo_asset_id,s.snapshot_id,s.job_id,s.snapshot_version,s.job_version,s.data_hash,s.payload_json,s.created_by,s.created_at FROM nzi_console.report_versions r JOIN nzi_console.reviewed_crp_snapshots s ON (s.organisation_id,s.snapshot_id)=(r.organisation_id,r.reviewed_snapshot_id) JOIN nzi_console.jobs j ON (j.organisation_id,j.job_id)=(r.organisation_id,r.job_id) WHERE r.report_version_id=$1 AND r.status IN ('validated','published','superseded') AND j.job_family='crp'`,[reportVersionId]);const row=rows[0];if(!row)return null;if(row.report_data_hash!==row.data_hash)throw new Error("Report version evidence hash does not match its reviewed snapshot.");return{reportVersionId:row.report_version_id,status:row.status,manifestVersion:row.manifest_version,...releaseFields(row),publishedAt:row.published_at==null?null:row.published_at instanceof Date?row.published_at.toISOString():String(row.published_at),dataHash:row.report_data_hash,snapshot:{id:row.snapshot_id,jobId:row.job_id,jobNumber:row.payload_json.jobNumber,client:row.payload_json.client,reportingYear:row.payload_json.reportingYear,version:row.snapshot_version,jobVersion:row.job_version,createdAt:row.created_at instanceof Date?row.created_at.toISOString():String(row.created_at),createdBy:row.created_by,dataHash:row.data_hash,target:row.payload_json.target??null,intensityTarget:row.payload_json.intensityTarget??null,annualComparison:row.payload_json.annualComparison??[],sections:row.payload_json.sections??resolveReportSections([]),gapResolutions:row.payload_json.gapResolutions??[],provenance:row.payload_json.provenance??null,measurements:row.payload_json.measurements}};}
export async function getCurrentPublishedCrpReport(db:Queryable,jobId:string):Promise<PublishedCrpReportReadModel|null>{const {rows}=await db.query<PublishedReportRow>(`SELECT r.report_version_id,r.manifest_version,r.data_hash AS report_data_hash,r.published_at,r.signee_name,r.signee_job_title,r.client_logo_asset_id,s.snapshot_id,s.job_id,s.snapshot_version,s.job_version,s.data_hash,s.payload_json,s.created_by,s.created_at FROM nzi_console.report_versions r JOIN nzi_console.reviewed_crp_snapshots s ON (s.organisation_id,s.snapshot_id)=(r.organisation_id,r.reviewed_snapshot_id) JOIN nzi_console.jobs j ON (j.organisation_id,j.job_id)=(r.organisation_id,r.job_id) WHERE r.job_id=$1 AND r.status='published' AND j.job_family='crp'`,[jobId]);const row=rows[0];if(!row)return null;if(row.report_data_hash!==row.data_hash)throw new Error("Published report evidence hash does not match its reviewed snapshot.");const snapshot:ReviewedCrpSnapshotReadModel={id:row.snapshot_id,jobId:row.job_id,jobNumber:row.payload_json.jobNumber,client:row.payload_json.client,reportingYear:row.payload_json.reportingYear,version:row.snapshot_version,jobVersion:row.job_version,createdAt:row.created_at instanceof Date?row.created_at.toISOString():String(row.created_at),createdBy:row.created_by,dataHash:row.data_hash,target:row.payload_json.target??null,intensityTarget:row.payload_json.intensityTarget??null,annualComparison:row.payload_json.annualComparison??[],sections:row.payload_json.sections??resolveReportSections([]),gapResolutions:row.payload_json.gapResolutions??[],provenance:row.payload_json.provenance??null,measurements:row.payload_json.measurements};return{reportVersionId:row.report_version_id,manifestVersion:row.manifest_version,publishedAt:row.published_at instanceof Date?row.published_at.toISOString():String(row.published_at),dataHash:row.report_data_hash,...releaseFields(row),snapshot};}
...
export async function listStaffRoleGovernance(db:Queryable):Promise<StaffRoleReadModel[]>{
  const [members,matrix]=await Promise.all([
    db.query<{role_id:StaffRole;members:string}>(`SELECT role_id,count(*) FILTER (WHERE status='active')::text AS members FROM nzi_console.memberships GROUP BY role_id`),
    db.query<{matrix_version:number;role_id:StaffRole;capability:string;scope:CapabilityScope}>(`SELECT matrix_version,role_id,capability,scope FROM nzi_console.staff_role_capabilities WHERE matrix_version=(SELECT max(matrix_version) FROM nzi_console.staff_capability_matrix_versions)`),
  ]);
  const counts=new Map(members.rows.map(row=>[row.role_id,Number(row.members)]));
  return staffRoles.map(role=>{
    const held=matrix.rows.filter(row=>row.role_id===role),names=new Set(held.map(row=>row.capability));
    return{id:role,name:roleLabels[role],members:counts.get(role)??0,matrixVersion:held[0]?.matrix_version??0,
      permissions:capabilities.filter(name=>names.has(name)).map(name=>held.find(row=>row.capability===name)?.scope==="own_clients"?`${name} (own)`:name),
      restricted:capabilities.filter(name=>!names.has(name))};
  });
}
