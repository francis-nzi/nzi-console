...
export async function listGrantedPortalJobs(db:Queryable,input:{portalUserId:string;clientId:string}){const {rows}=await db.query<{job_id:string;job_number:string;title:string;reporting_year:number|null;has_published_report:boolean}>(`SELECT j.job_id,j.job_number,j.title,j.reporting_year,EXISTS(SELECT 1 FROM nzi_console.report_versions r WHERE (r.organisation_id,r.job_id)=(j.organisation_id,j.job_id) AND r.status='published') AS has_published_report FROM nzi_console.portal_access_grants g JOIN nzi_console.jobs j ON (j.organisation_id,j.job_id,j.client_id)=(g.organisation_id,g.job_id,g.client_id) WHERE g.portal_user_id=$1 AND g.client_id=$2 AND g.revoked_at IS NULL ORDER BY j.sequence DESC`,[input.portalUserId,input.clientId]);return rows.map(row=>({id:row.job_id,number:row.job_number,title:row.title,reportingYear:row.reporting_year,hasPublishedReport:row.has_published_report}));}
