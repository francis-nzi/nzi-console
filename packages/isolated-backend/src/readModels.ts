...
import type { DatasetOption, EmissionsTargetReadModel, FactorOption, IntensityTargetReadModel, PublishedCrpReportReadModel, PurchasedGoodsCategoryOption, ReviewedCrpSnapshotReadModel, SiteOption, ScopeQaReadiness, ScopeQualityTier, ScopeRowReadModel } from "@nzi/contracts";
...
export async function listScopeRows(db: Queryable, jobId: string): Promise<ScopeRowReadModel[]> {
  ...
  return rows.map((row) => ({ id: row.scope_row_id, jobId: row.job_id, scope: row.scope, sourceLabel: row.source_label,siteId:row.site_id,siteLabel:row.site_label,purchasedGoodsCategoryId:row.purchased_goods_category_id,purchasedGoodsCategoryLabel:row.purchased_goods_category_label,
...
export async function listGrantedPortalJobs(db:Queryable,input:{portalUserId:string;clientId:string}){const {rows}=await db.query<{job_id:string;job_number:string;title:string;reporting_year:number|null;report_version_id:string|null;approved_at:Date|string|null;latest_review_author:"portal"|"staff"|null}>(`SELECT j.job_id,j.job_number,j.title,j.reporting_year,pr.report_version_id,a.approved_at,(SELECT m.author_principal FROM nzi_console.portal_report_comments m WHERE (m.organisation_id,m.report_version_id)=(j.organisation_id,pr.report_version_id) ORDER BY m.created_at DESC,m.comment_id DESC LIMIT 1) AS latest_review_author FROM nzi_console.portal_access_grants g JOIN nzi_console.jobs j ON (j.organisation_id,j.job_id,j.client_id)=(g.organisation_id,g.job_id,g.client_id) LEFT JOIN LATERAL (SELECT r.report_version_id FROM nzi_console.report_versions r WHERE (r.organisation_id,r.job_id)=(j.organisation_id,j.job_id) AND r.status='published' LIMIT 1) pr ON true LEFT JOIN nzi_console.portal_report_approvals a ON (a.organisation_id,a.report_version_id,a.portal_user_id)=(j.organisation_id,pr.report_version_id,g.portal_user_id) WHERE g.portal_user_id=$1 AND g.client_id=$2 AND g.revoked_at IS NULL ORDER BY j.sequence DESC`,[input.portalUserId,input.clientId]);return rows.map(row=>({id:row.job_id,number:row.job_number,title:row.title,reportingYear:row.reporting_year,hasPublishedReport:row.report_version_id!==null,approved:row.approved_at!==null,approvedAt:row.approved_at===null?null:row.approved_at instanceof Date?row.approved_at.toISOString():String(row.approved_at),latestReviewAuthor:row.latest_review_author}));}
