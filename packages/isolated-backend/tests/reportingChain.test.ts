...
function chainPool(opts: {
  ...
  const client = {
    async query(sql: string) {
      ...
      if (sql.includes("FROM nzi_console.jobs j")) return { rows: opts.jobFamily === undefined || opts.jobFamily === "crp" ? [{ client_id: "client-a", reporting_year: opts.reportingYear ?? 2026, start_date: "2026-01-01", job_family: "crp", period_from: null, period_to: null, baseline_period_end: null }] : opts.jobFamily === null ? [] : [{ client_id: "client-a", reporting_year: 2026, start_date: "2026-01-01", job_family: opts.jobFamily, period_from: null, period_to: null, baseline_period_end: null }] };
      if (sql.includes("job_emissions_targets")) return { rows: opts.baselineYear == null ? [] : [{ baseline_year: opts.baselineYear }] };
      if (sql.includes("DISTINCT ON") && sql.includes("reviewed_crp_snapshots")) return { rows: (opts.priors ?? []).map((p) => ({ snapshot_id: p.id, data_hash: `sha256:${p.year}`, reporting_year: p.year, period_from: null, period_to: null })) };
