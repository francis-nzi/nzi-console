...
describe("isolated Postgres adapter", () => {
  ...
  it("distinguishes source review from canonical row sync and review status",async()=>{const db={query:async(sql:string)=>({rows:sql.includes("job_emission_groups")?[]:[{source_id:"source-a",job_id:"job-a",group_id:null,scope:"3.7",source_type:"commuting",source_subtype:null,site_id:null,source_name:"Employee A",asset_identifier:"EMP-001",dataset_id:"dataset-a",factor_id:"factor-a",factor_source:"dataset",client_factor_id:null,quantity:"1200",unit:"km",apply_pct:"100",data_source:"HR survey",data_confidence:"M",monthly_activity_json:[],detail_json:{kind:"commuting"},notes:null,calculated_tco2e:null,enabled:true,submitted_by_portal:false,review_status:null,version:1,scope_row_id:"row-a",scope_row_version:3,scope_row_review_status:"pending"}]})} as Queryable;const register=await listJobEmissionSourceRegister(db,"job-a"),source=register.sources[0]!;assert.equal(source.reviewStatus,null);assert.equal(source.scopeRowId,"row-a");assert.equal(source.scopeRowVersion,3);assert.equal(source.scopeRowReviewStatus,"pending");assert.equal(source.rolledForwardFromSourceId,null);assert.equal(source.factorVersionMoved,false);assert.equal(source.yoyPriorQuantity,null);});

  it("carries the prior year's quantity and a moved-version flag on a rolled-forward source",async()=>{const db={query:async(sql:string)=>({rows:sql.includes("job_emission_groups")?[]:[{source_id:"rf-1",job_id:"job-2026",group_id:null,scope:"3.1",source_type:"spend",source_subtype:"7504",site_id:null,source_name:"Office paper",asset_identifier:null,purchased_goods_category_id:"pgs-paper",rolled_forward_from_source_id:"prior-1",factor_version_moved:true,yoy_prior_quantity:"1240",yoy_prior_unit:"GBP",dataset_id:"ds-2025",factor_id:"f-paper",factor_source:"dataset",client_factor_id:null,quantity:"3900",unit:"GBP",apply_pct:"100",data_source:"Rolled forward from J000700 · FY2025",data_confidence:null,monthly_activity_json:[],detail_json:{kind:"spend"},notes:null,calculated_tco2e:null,enabled:true,submitted_by_portal:false,review_status:"pending",version:1,scope_row_id:null,scope_row_version:null,scope_row_review_status:null}]})} as Queryable;const register=await listJobEmissionSourceRegister(db,"job-2026"),source=register.sources[0]!;assert.equal(source.rolledForwardFromSourceId,"prior-1");assert.equal(source.factorVersionMoved,true);assert.equal(source.yoyPriorQuantity,1240);assert.equal(source.yoyPriorUnit,"GBP");});
