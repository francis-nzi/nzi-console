...
import { getCurrentPublishedCrpReport,getGrantedPublishedCrpReport,listClients,listGrantedPortalJobs, listJobs, listScopeRows, withTenantRead, type Queryable } from "../src/index";
...
describe("isolated Postgres adapter", () => {
  ...
  it("surfaces approval and latest NZI response state on the portal job list",async()=>{const db={query:async()=>({rows:[{job_id:"job-a",job_number:"J000612",title:"Synthetic CRP",reporting_year:2026,report_version_id:"report-a",approved_at:"2026-08-25T18:00:00Z",latest_review_author:"staff"}]})} as Queryable;const jobs=await listGrantedPortalJobs(db,{portalUserId:"portal-a",clientId:"client-a"});assert.equal(jobs[0]?.approved,true);assert.equal(jobs[0]?.approvedAt,"2026-08-25T18:00:00Z");assert.equal(jobs[0]?.latestReviewAuthor,"staff");});
