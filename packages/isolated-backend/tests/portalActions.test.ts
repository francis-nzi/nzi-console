...
describe("portal A2-lite action tracker", () => {
  ...
  it("creates and version-updates qualitative progress with an audit trail", async () => {
    ...
    const createAudit = createdDb.calls.find((call) => call.sql.includes("portal.tracker.action.create"));
    assert.match(createAudit?.sql ?? "", /\$5::jsonb/);
    assert.deepEqual(JSON.parse(String(createAudit?.values?.[4])), { jobId: "job-1", leverCode: "C2.4", progressPercent: 20 });
    ...
    const updateAudit = updatedDb.calls.find((call) => call.sql.includes("portal.tracker.action.update"));
    assert.deepEqual(JSON.parse(String(updateAudit?.values?.[4])), { jobId: "job-1", leverCode: "C2.4", progressPercent: 60, version: 2 });
    ...
    const deleteAudit = deletedDb.calls.find((call) => call.sql.includes("portal.tracker.action.delete"));
    assert.deepEqual(JSON.parse(String(deleteAudit?.values?.[4])), { jobId: "job-1" });
  });
});
